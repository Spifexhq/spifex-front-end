...
const SettlementModal: React.FC<SettlementModalProps> = ({ isOpen, onClose, selectedEntries, onSave }) => {
  ...
  useEffect(() => {
    const mapped = selectedEntries.map((e) => ({
      id: e.id,
      due_date: new Date(e.due_date) > new Date() ? todayISO : e.due_date,
      description: e.description,
      amount: e.amount,
    }));
    setEntriesState(mapped);
  }, [selectedEntries, todayISO]);
  ...
  const handleLiquidate = async () => {
    try {
      // TODO: enviar selectedBankIds + entriesState para endpoint de liquidação
      // await api.liquidate({ bank_ids: selectedBankIds, entries: entriesState });
      onSave();
      onClose();
    } catch (err) {
