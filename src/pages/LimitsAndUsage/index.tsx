 ...
 * i18n:  group "limits" (fallbacks inline)
 * ---------------------------------------------------------------------------*/
...
type LimitsItem = {
  permission: { code: string; name: string | null; category: string | null; description: string | null };
...
/* --------------------------------- UI bits -------------------------------- */
const ToneBadge: React.FC<{ tone?: "green" | "lime" | "amber" | "orange" | "red" | "gray"; children: React.ReactNode }> = ({
  tone = "gray",
  children
}) => {
  ...
  return <span className={`text-[11px] px-2 py-0.5 rounded-full border ${tones[tone]} inline-flex items-center`}>{children}</span>;
};

const Progress: React.FC<{ pct: number; tone: "green" | "lime" | "amber" | "orange" | "red" | "gray" }> = ({ pct, tone }) => {
...
const LimitsAndUsage: React.FC = () => {
  ...
  useEffect(() => { document.title = t("limits:title", "Limits & Usage"); }, [t]);
  ...
  const filtered = useMemo(() => {
    ...
    const base = !qq
      ...
      : data.items.filter(it =>
          (it.permission.code?.toLowerCase() || "").includes(qq) ||
          (it.permission.name?.toLowerCase() || "").includes(qq) ||
          (it.permission.category?.toLowerCase() || "").includes(qq)
        );
    ...
    return enriched.sort((a, b) => {
      const A =
        sortKey === "permission" ? a.permission.code :
        sortKey === "period"     ? a.limits.period :
        sortKey === "limit"      ? (a.limits.limit ?? 0) :
        sortKey === "used"       ? a.usage.used :
                                   a._pct;
      const B =
        sortKey === "permission" ? b.permission.code :
        sortKey === "period"     ? b.limits.period :
        sortKey === "limit"      ? (b.limits.limit ?? 0) :
        sortKey === "used"       ? b.usage.used :
                                   b._pct;
  ...
  if (loading) return (<><TopProgress active variant="top" topOffset={64} /><PageSkeleton rows={6} /></>);
        ...
        <header className="flex items-center justify-between mb-4">
          <div>
            <div className="text-[10px] uppercase tracking-wide text-gray-600">
              {t("limits:header.kicker", "Subscription")}
            </div>
            <h1 className="text-[16px] font-semibold text-gray-900">
              {t("limits:header.title", "Limits & Usage")}
            </h1>
            <p className="text-[12px] text-gray-600">
              {t("limits:header.subtitle", "Current plan:")}{" "}
              <span className="font-medium">{data?.plan?.name ?? "—"}</span>
            </p>
            {/* Legend */}
            <div className="mt-2 flex flex-wrap gap-2">
              <ToneBadge tone="green">{t("limits:legend.ok", "OK < 70%")}</ToneBadge>
              <ToneBadge tone="amber">{t("limits:legend.warn", "70–89%")}</ToneBadge>
              <ToneBadge tone="orange">{t("limits:legend.high", "90–99%")}</ToneBadge>
              <ToneBadge tone="red">{t("limits:legend.exhausted", "100% Exhausted")}</ToneBadge>
              <ToneBadge tone="lime">{t("limits:legend.unmetered", "Unmetered")}</ToneBadge>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder={t("limits:search", "Search permission…")}
          ...
          <div className="divide-y divide-gray-200">
              ...
              return (
                <div key={`${permission.code}-${limits.period}`} className="grid grid-cols-12 items-center px-4 py-3">
                  ...
                  <div className="col-span-4">
                    <div className="flex items-center gap-2">
                      <div className="text-[13px] font-medium text-gray-900">{permission.code}</div>
                      <ToneBadge tone={st.tone}>
                        {limits.unmetered
                          ? t("limits:status.unmetered", "Unmetered")
                          : st.key === "exhausted"
                          ? t("limits:status.exhausted", "Exhausted")
                          : st.key === "high"
                          ? t("limits:status.high", "High")
                          : st.key === "warn"
                          ? t("limits:status.warning", "Warning")
                          : t("limits:status.ok", "OK")}
                      </ToneBadge>
                    </div>
                    <div className="text-[12px] text-gray-600">
                      {permission.name || "—"}{permission.category ? ` • ${permission.category}` : ""}
                    </div>
                    {permission.description && (
                      <div className="mt-0.5 text-[11px] text-gray-500 line-clamp-2">{permission.description}</div>
                    )}
                  </div>
                  ...
                  <div className="col-span-2 text-[13px] text-gray-800">
                    ...
                    {!limits.unmetered && usage.resets_at && (
                      <div className="text-[11px] text-gray-500">
                        {t("limits:resets", "Resets")}: {new Date(usage.resets_at).toLocaleString()}
                      </div>
                    )}
                  </div>
                  ...
                  {/* Limit */}
                  <div className="col-span-2 text-[13px] text-gray-800">
                    {limits.unmetered ? t("limits:unmetered", "Unmetered") : (limits.limit ?? 0)}
                    {limits.enforce === false && (
                      <div className="text-[11px] text-gray-500">{t("limits:notEnforced", "Not enforced")}</div>
                    )}
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    {limits.unmetered ? (
                      <div className="text-[11px] text-gray-500">{t("limits:infinite", "No cap")}</div>
                    ) : (
                      <div className="text-[11px] text-gray-500">
                        {t("limits:remaining", "Remaining")}: {usage.remaining ?? 0}
                      </div>
                    )}
                  </div>
            ...
            {!filtered.length && (
              <div className="px-4 py-6 text-[13px] text-gray-600">
                {t("limits:empty", "No permissions match your search.")}
              </div>
            )}
          </div>
        </div>
