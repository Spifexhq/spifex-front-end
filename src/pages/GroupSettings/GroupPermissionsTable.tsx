...
const GroupPermissionsTable: React.FC<GroupPermissionsTableProps> = ({
  ...
  const canChangeGroup = useMemo(() => {
    ...
    return permissions.includes("change_group");
  }, [isOwner, permissions]);

  const selectionDisabled = !!disabled || !canChangeGroup;
    ...
    <PermissionMiddleware codeName={["view_permission", "view_group"]} requireAll>
      <div className="border-t border-gray-200">
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  key={tab.id}
                  type="button"
                  onClick={() => !disabled && setActiveTab(tab.id)} // ✅ keep as-is (tabs only respect `disabled`)
            ...
            return (
              <div
                ...
                className={`flex items-start justify-between px-4 py-3 transition-colors ${
                  selectionDisabled ? "cursor-default" : "cursor-pointer hover:bg-gray-50"
                }`}
                onClick={() => {
        ...
        <div className="px-4 py-3 border-t border-gray-200 flex justify-end gap-2">
          <Button variant="cancel" type="button" onClick={onUndo} disabled={!dirty || selectionDisabled}>
            {t("buttons.undo")}
          </Button>
          <Button type="button" onClick={onSave} disabled={!dirty || selectionDisabled}>
            {t("buttons.save")}
          </Button>
        </div>
      </div>
